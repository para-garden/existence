...
- [PENDING] `js/ui.js:866-869` — render-time mutation: `renderPhone()` sets `msg.read = true`, writes `friend_contact` timestamps, calls `adjustSentiment(threadContact, 'guilt', -0.02)`. Simulation state changes from inside the renderer; not in action log; replay-incoherent. — emit `read_thread` interaction; move logic to `execute()` _(severity: high)_
