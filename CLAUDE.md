...
**Injuries are first-class, not sub-fields of their causes.** A levator ani avulsion caused by a delivery and a torn ACL caused by a fall are both injuries. Cause is context on the injury record, not a reason to nest the injury inside the causing event. `injury_history: [{ type, onset_time, severity, cause, resolved }]` — one system, all injuries. Don't let the cause determine where the injury lives.
