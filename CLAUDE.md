...
- **When fixes keep missing, stop patching and ask "what IS this?"** If you've proposed several fixes and they all get rejected or feel wrong, you are working inside the wrong frame. The escape is not a better patch — it's the question: what is this thing, in full generality? The alarm case: three proposed fixes (reset location, set flag in sleep path, event log) all missed because the question was "where should alarm_went_off go?" when the right question was "why does an alarm fire?" → time-threshold event → scheduling system → the whole frame changes and alarm_went_off doesn't need to exist. This applies in debugging, investigation, and design: if the right answer keeps escaping you, you have not yet asked the right question.
