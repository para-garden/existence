...
4. ~~**Full clothing implementation**~~ — **DONE.** `js/clothing.js` (full_v1): per-item tracking, wardrobe from chargen, fit states, `outfitDescription()`, `floorDescription()` with item names, `resolveUndressDestination()` (1 RNG call, balanced), `startWash()`. `js/body.js` created (minimum viable interface). Body params at chargen: ASAB, puberty_history, hrt_history, constitutional_conditions, breast_tissue_score, abdominal_baseline. `pregnancy_week`/`binder_start_time` added to state defaults. **Remaining debts:** (a) get_dressed prose still uses outfit_* sets — update content.js to call `outfitDescription()`; (b) fit is `comfortable` for all items until Body.dimensionAtTime() is wired into wardrobe generation; (c) conscientiousness param not yet added; (d) RunRecord version bump / subsystem_versions not yet done.
