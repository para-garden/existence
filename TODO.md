...
- **Pregnancy and contraception** — `pregnancy_week` state var and body.js modifiers stubbed. Full model needs:
  ...
  - **Reproductive history as condition carrier.** Several conditions that arise during pregnancy are more likely and more severe in subsequent pregnancies, especially if they occurred before. This is the same pattern as post-maturity: the right model is `reproductive_history: [{ outcome, gestational_weeks, conditions: [...] }]` — not just a pregnancy count. Conditions in this class:
    ...
    - **Pelvic floor dysfunction** (stress incontinence, prolapse) — risk roughly doubles with each vaginal delivery on average, but this is not uniform: **connective tissue genetics** are the dominant modifier and belong at chargen as a constitutional parameter (heritable; people with dense, low-relaxin-sensitivity connective tissue have much lower risk regardless of delivery count). **Levator ani avulsion** (pubococcygeus tearing from pelvic sidewall, ~13–30% of vaginal deliveries, higher with forceps/vacuum) is a discrete injury event at delivery — a per-delivery PRNG draw, not a smooth accumulation — and is a major cause of prolapse specifically; largely irreversible without surgery. General pelvic floor fatigue (accumulation model) and avulsion injury (discrete event model) are distinct mechanisms contributing to the same condition. Other modifiers: delivery type (elective C-section substantially reduces risk), pushing stage length and technique, baby size, pelvic floor PT pre/during pregnancy. Post-partum hypoestrogenic state during breastfeeding reduces tissue strength and slows recovery (reversible). Prior occurrence remains strongest predictor of recurrence.
