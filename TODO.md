...
**Within-day affect variability is ~12× too flat — architectural, not a coupling magnitude.** The NT-trajectory harness measures within-day affect iSD ~1.0 (ordinary day) vs the empirical 8–18 band (PMID 32324001). The tuning phase (2026-06-05) proved this is NOT reachable by scaling SER/DA couplings: even both targets oscillating ±20/day caps affect iSD at ~5.2, because serotonin's drift rate (`ntRates.serotonin = [0.06,0.08]`, t½ 9–11h, literature-anchored) low-passes within-day serotonin-target variance to near-zero on the 0.6-weighted affect proxy. Three fixes, in leverage order (full analysis: `docs/research/mood-variability.md` § TUNING-PHASE OUTCOME):
> 1. **Event-driven affect perturbations** — discrete interactions/events should apply transient acute NT excursions on the *fast* systems (DA/NE) so the smooth-driver tonic floor (~1, correctly low) is joined by the momentary component that dominates real EMA variance. Harness should then validate a free-running gameplay trajectory, not only re-pinned smooth drivers.
> 2. **A reactivity/amplitude personality axis distinct from `effectiveInertia()`.** The archetype iSD spread is backward: the sim's inertia (rate) axis makes high-neuroticism characters *less* volatile, but neuroticism is associated with *greater* affective variability simultaneously with greater inertia — the stability–instability paradox (Mader 2023, PNAS, PMID 37253012, PMC10266024). Add an axis that scales target-excursion magnitude by neuroticism/rumination, independent of drift rate.
> 3. The momentary-affect metric (harness `affectProxy`, currently 0.6 SER / 0.4 DA from `moodTone()`'s tone gating) should weight the fast systems over tonic serotonin — `moodTone()` is sustained-tone, not momentary affect.
