...
Canonical design: `docs/design/simulated-life.md`, especially §10 "wrong-turns graveyard" (now W1–W12), **§11 "The Generator/Evaluator — resolved,"** and the new **§12 "The Act-Emitter — resolved mechanism, open fidelity."** The direction is a multi-agent forward simulation from a chosen t=0; memory (Stage 1) and body-image work ultimately read from this. Large, multi-phase — a direction, not an imminent build.
...
**Compute tractability — RESOLVED (measured tractable).** Forward-sim from t=0 was measured (`scripts/forward-sim-spike.js`, results `docs/artifacts/emitter-design/spike-results.md`): central case (N=300, 10⁵ acts, indexed) = **0.22 s**, stress (N=1000, 10⁶ acts) = **~2.4 s**, 2–3 orders of magnitude under a 5 s interactive budget. **FOLD (the ~28-system drift loop), not affordance enumeration, dominates** (~85–90% of per-act cost). The "intractable" premise that drove every §10 wrong turn is **false** (META-TRAP empirical clause corrected). Caveat: the spike measures pipeline *shape* + fold cost, **not propensity fidelity** — trust the order of magnitude, not the point estimate.

**Act-emitter MECHANISM — RESOLVED into §12** ("The Act-Emitter — resolved mechanism, open fidelity"). Pipeline FOLD → AFFORD → WEIGHT → DRAW → COMMIT = the existing dynamical engine run forward (not a new model, not an LLM, not W9). Refinement A: procedural/skill organ is a **derived fold-statistic** over the node's own act-stream (fixes "everyone is the same person at different NT setpoints"; restores cognitive fairness). Refinement B (verified `js/habits.js`): the shipped CART predicts/shades, never selects — the human is the emitter in the policy slot, NPCs run the seeded propensity emitter in the same slot. Refinement C: coefficient-provenance discipline (`anchor`/`debt`/`population`-as-declared-W9), the W9 firewall keyed on calibration target, not function form (folded into the W9 graveyard entry).

Open questions that remain:

- **The propensity/hazard FIDELITY is the real remaining crux** — the actual weight and gate functions and the ActKind/verb ontology (§12C, §12E). Cost is answered; "emitter and cost are the same question" is now narrowed to fidelity. Must be **designed, not retreated-to-CART** (W9).
- **A follow-up compute spike with the REAL gate + REAL fold** — to convert the order-of-magnitude verdict into a point estimate and confirm the margin survives a heavy gate + heavy fold (spike-results.md §8).
...
- **Substance P → NT targets and sleep** — wired: `substancePTarget()` registered in ntTargetFns. Elevated SP raises cortisolTarget (max +6 at SP=100; Blackburn-Munro 2001 PMID 11731074), reduces serotoninTarget (max −6; Stahl 2002 PMID 12185833), and multiplies sleep qualityMult via `painSleepInterference()` (0.88× at max; Buysse 2010 PMID 20040399). All 5 sleep sites in content.js wired. SP is elevated by: active migraine (+25), hEDS joint events (+15), hEDS constitutional (+12); suppressed by opioids. `grep 'Approximation debt (substance_p)'` for all magnitude debts.
...
- **Sponsor relationship** — implemented: named sponsor NPC at meeting 10, call_sponsor/text_sponsor/meet_with_sponsor interactions, craving reduction, relapse-aware prose, step work progression (recovery_step 0–12, recoveryStepTier(), step-aware craving modifiers), sponsor backstory (sponsor_years_sober, sponsor_substance, sponsor_communication_style at chargen), style-aware prose, 4 backstory idle thoughts. `grep 'Approximation debt (recovery)'` for remaining calibration debts.
...
Remaining:
- **Race/ethnicity effects** — implemented: intersectional pay gap (racial wage multiplier × gender pay gap), code-switching fatigue, diagnostic disparities (pain undertreatment modifier in `see_doctor_clinic` and `er_treatment`; direction from Hoffman 2016 PMID 27044069), housing discrimination (callback gap in `apply_for_apartment`; Pager & Shepherd 2008 DOI 10.1146/annurev.soc.34.040507.134833), 8 idle thoughts (4 Black, 4 non-white). `grep 'Approximation debt (race/ethnicity)'`, `grep 'Approximation debt (racial pay gap)'`. Note: BLS 2024 overall gender pay gap is 83% (code uses 82% — close); AAUW 2024 Black women 65¢, Latina 58¢ (code notes 63¢/57¢ — slightly outdated but in range). Update `(racial pay gap)` sites when calibrating chargen.js.
