...
- **Pregnancy and contraception** — `pregnancy_week` state var and body.js modifiers stubbed. Full model needs:
  ...
  - **Reproductive history vs. current body state.** Conditions arising from pregnancy are persistent — they don't end when the pregnancy does. The right split: `reproductive_history` records **delivery event circumstances** (`[{ outcome, gestational_weeks, delivery_type, avulsion, instrumented, birth_weight }]`) — discrete events that explain how current state got that way. Persistent conditions (diastasis severity, pelvic floor strength) live in **current character state**, modified by those events, readable by prose and by future pregnancy risk calculations. Don't put conditions on history entries — that implies they ended with the pregnancy. Risk for the next pregnancy reads current state, not archived history. Note: `avulsion` belongs on the delivery event record because it's a discrete irreversible injury that occurred at a specific moment; it's also reflected in current pelvic_floor state, but the event record is what future deliveries need to assess cumulative damage. Conditions in this class:
