...
- ~~**Hygiene and shower modeling (first pass)**~~ — **DONE.** Four shower interactions: `quick_shower` (6 min, always available), `shower` (15 min + deterministic extension, not depleted), `long_shower` (25 min + extension, not depleted), `cold_shower` (8 min, always available). Duration/temperature are player choices via distinct labeled interactions. Compulsive extension: `shower` and `long_shower` run longer if high NE + low GABA + rumination (deterministic formula, no RNG). Cold shower NT effects: NE+6, cortisol+3, adenosine-10, energy+5. Warm showers: GABA+1–5, cortisol-2–8, NE-1–3 scaling with duration. Extension prose acknowledgment appended deterministically. Approximation debt: extension coefficients (0.5/0.3/0.2), NT effect magnitudes all chosen not derived.
  ...
  **Remaining:**
  ...
  - ~~**Shower as phone-free moment**~~ — **DONE.** All four shower interactions have phone-aware weighted variants (unread/guilt/in_need signals). Bathroom description adds post-shower phone note when unread messages exist. `check_phone_bathroom` captures the immediate-reach-for-it gesture with differentiated prose: "you reach for it before you've dried off" (in_need), "before you've put the towel down" (guilt+unread), "nothing new. you checked anyway" (compulsive check, nothing there).
...
4. ~~**Full clothing implementation**~~ — **DONE.** `js/clothing.js` (full_v1): per-item tracking, wardrobe from chargen, fit states, `outfitDescription()`, `floorDescription()` with item names, `startWash()`. Explicit undress destination interactions (undress_floor / undress_chair / undress_basket). Sleep auto-undresses only if still dressed. `js/body.js` created (minimum viable interface). Body params at chargen: ASAB, puberty_history, hrt_history, constitutional_conditions, breast_tissue_score, abdominal_baseline. `pregnancy_week`/`binder_start_time` added to state defaults. ~~**Debt (a):**~~ **DONE.** `get_dressed` prose now calls `outfitDescription()` — actual item names with mood/energy/stress framing. `outfitSets` removed from chargen.js (1 charRng call removed; safe since characters stored verbatim). Chargen sandbox wardrobe preview replaced with static item-derived text. **Remaining debts:** (b) fit is `comfortable` for all items until Body.dimensionAtTime() is wired into wardrobe generation; (c) conscientiousness param not yet added; (d) RunRecord version bump / subsystem_versions not yet done; (e) **bathroom towel bar / shelf** — if apartment has a surface near the sink, undressing for shower could land on `bathroom_shelf` instead of `floor_bathroom`. Needs apartment features system (see note below); (f) **wearState doesn't age during sleep** — items worn to bed don't progress (worn_once → worn_out) until the next explicit undress. Correct if sleep is brief/partial; underestimates wear on items slept in full nights.
...
6. **Apartment features** — physical amenities that vary by unit: towel bar / shelf near sink, clothing rack in bedroom, in-unit laundry, dishwasher, etc. These gate interactions and affect where things end up (bathroom undress could land on `bathroom_shelf` if the character has a surface near the sink). **Critical:** apartment features cannot be derived from rent alone. Rent correlates with apartment quality but the relationship is weak and noisy — it depends heavily on cost of living (same $1200/mo = studio in NYC, 2BR in the midwest), and is broken entirely by non-market arrangements (craigslist underpricing, family discount, informal subletting, inherited tenancy). The real driver is a separate `apartment_features` or `housing_quality` variable, generated at chargen as its own thing from backstory signals — economic_origin as a prior, but with independent noise so a precarious character can have a surprisingly nice place (a friend's couch upgrade, inheriting a good unit) and a comfortable character can have a terrible one (high-cost city eating their income). Don't proxy housing quality through rent.
...
**All creation modes are views into the same mechanism.** The life history simulation is the single source of truth. All modes call the same `simulateFinancialHistory` / `generateBackstory` / etc. machinery — they differ only in how much the player controls the inputs vs. the outputs:

- **Random:** simulation runs unconstrained. Player gets whoever it produces.
- **Lived-in:** player authors the history (past decision forks). Outputs emerge from choices.
- **Sandbox:** player specifies desired current-state outputs. Simulation reverse-engineers a plausible history that produces them.

Sandbox is the interesting case. It's not "bypass history" — it's **inverse simulation**: given desired outputs, find backstory inputs that produce them. For things the sim models well (money, job standing, financial anxiety, pay rate, labor arrangement), the inverse is tight — specific combinations of economic origin, career stability, and life events reliably produce those values. For things still drawn arbitrarily (neuroticism, introversion), the inverse is looser — the sim picks life events that push in the right direction and acknowledges the remaining gap. The "hack" framing is intentional: it's approximate, not exact, because the forward simulation has noise. The character's history might not be the *only* history that gets there, but it's *a* history — coherent, specific, theirs.

As the forward simulation deepens, the inverse becomes tighter. The modes converge: when the sim is deep enough, "random" produces someone you feel you've met before, and "sandbox" produces someone who genuinely had the life that made them this way.
