...
- **Physical therapy exercises** — injury recovery pathway; distinct from stretch_morning (remedial, prescribed, painful before helpful). Needs `injury_history` (now implemented: `addInjury()`, `resolveInjury()`, `currentInjuries()`, `hasInjury()` in state.js; schema `{ type, onset_time, severity, cause, resolved }`). Interaction needs: prescription from clinic appointment, graduated pain/progress, daily streak tracking.
...
**Remaining:**
- **Sound lexical coverage** — passes complete. All acoustic sources have body layers. Smell/temperature modifier pools expanded (2026-02-25): `outdoor_temperature` +3, `dishes_smell` +3, `cold_air_smell` +2, `office_ambient_smell` +3 (null weight 3.0→2.0). No remaining thin spots identified. Acoustic dimension taxonomies are not the right approach.
...
- **POTS / hEDS / MCAS** — hEDS implemented (laxity >= 88 at chargen; chronic pain drift; POTS comorbidity 50% roll; joint/pain idle thoughts). MCAS implemented: 40% comorbidity roll when hEDS (Akin 2021 PMID 34199069); cleaning smell triggers nausea sensitivity (0.5 pt/hr when smell_intensity > 50); 5 idle thoughts (flushing, GI upset, skin awareness, throat itch). Trigger catalog now covers: smell (cleaning/food/coffee), heat (>28°C), cold (<13°C), stress (cortisol>65), exercise (NE>70 proxy — conflates with anxiety, noted debt). `grep 'Approximation debt (MCAS)'` in state.js for remaining magnitude debts.
...
Dental pain — treatment mechanic implemented: `dental_condition` worsening (inflamed→infected→abscess), `schedule_dentist` phone interaction, `dentist_appointment` event, `dentalConditionTier()` exported. Tooth loss end-state (2026-02-24): `dental_abscess_onset` tracked; after 30 game-days at abscess tier, `tooth_extraction` interrupt fires → emergency extraction event, `teeth_lost` counter, injury record, `dental_ache` → 75. Remaining debts:
...
**Phone power** — battery drain, charging, battery health degradation, and signal bars implemented. `charge_phone` gates on `effectiveBatteryMax()`. Signal variation by weather implemented (drizzle/snow → signal 2 at exposed outdoor locations; 5% signal-2 drop rate added to call_friend/call_family). Future: charge rate by charger type.
