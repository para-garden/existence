...
- **Interactions:** attend_therapy (80 min, $150, 2 RNG, rapport-scaled NT effects), skip_therapy (1 RNG, rapport −3, stress +2), cancel_therapy (1 RNG, clears therapy_active + interrupt).
- **Idle thoughts:** 6 variants gated on therapy_active — appointment day awareness, post-session processing, telling-someone awkwardness, rapport-dependent relationship texture.
- `grep 'Approximation debt (therapy)'` — session cost, referral threshold, rapport growth rate, serotonin target coefficient.
...
## Interactions (184 + dynamic item interactions)
...
### Clinic (4)
check_in_clinic (not checked in + not clinic_ready; 5 min; cortisol +3, serotonin −1; schedules clinic_ready interrupt 45–89 min out; 2 RNG calls), wait_at_clinic (checked in + not clinic_ready; 10 min; energy −2; adjustSentiment routine/irritation +0.005; 2 RNG calls), see_doctor_clinic (checked in + clinic_ready OR 45+ min elapsed; 20 min; branches on health state — dental referral + dentist scheduled + dental_ache −25; antacid prescription for gastritis; HRT script for trans+hrt_active; pain_management note for chronic_pain > 40; therapy referral when serotonin < 40 + not already in therapy; general serotonin +5 cortisol −8; 3 RNG calls unconditional), leave_clinic (always available; 15 min; clears check-in state, moves to street; 1 RNG call).
State: `clinic_last_visit` (game-minutes), `clinic_checkin_time` (null or game-minutes), `clinic_ready` (bool), `clinic_prescriptions` (string[]: 'antacid' | 'hrt' | 'dental_referral' | 'pain_management' | 'therapy_referral').
Prescription effects: 'antacid' → gastritis_pain decays 0.1/hr faster in advanceTime(). 'hrt' → enables take_hrt interaction (already modeled). 'dental_referral' → dentist appointment scheduled on interrupt queue. 'therapy_referral' → therapy_active set, therapy_appointment interrupt scheduled weekly.
