...
- **Condition worsening:** `dental_condition` ('sound'|'inflamed'|'infected'|'abscess') + `dental_last_treated` timestamp. `inflamed`→`infected` after 14 game-days untreated (+20 ache); `infected`→`abscess` after 7 further days (+30 ache). Abscess: nausea +0.3/hr, cortisol +0.5/hr (systemic). Approximation debt (dental): all timelines chosen.
