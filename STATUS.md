...
- Identity dimensions: pronouns, trans status, trans_presentation, hrt_active, sexuality, out_at_work, out_to_family (4 unconditional charRng calls; derived deterministically from existing vars where possible)

**Identity system (foundation layer):** Chargen generates pronouns (she/her 48%, he/him 48%, they/them 3%, she/they or he/they 1%), trans (0.8% — Williams Institute 2022 approximate; PMID unverified), HRT (65% of trans characters; approximate unverified), sexuality (straight 90%, bisexual 6%, gay 4%; approximate). Out status derived deterministically from neuroticism + financial_anxiety (no new charRng calls). First-order effects: HRT NT effects (serotoninTarget +5/gabaTarget +3 for transfem when taken regularly; norepinephrineTarget +3 for transmasc; mood instability when missed); closet energy cost (0.4 pts/hr social_energy at workplace when not out + sexuality non-straight; 0.3 pts/hr additional for trans stealth). Identity texture in idle thoughts: HRT reminder when overdue, closet texture at work when not out, at-home relief.

**Structural discrimination (first-order, 2026-02-25):** Ambient effects only — no discrete discrimination events. All sites marked `// Approximation debt (structural discrimination)`.
- **Gender pay gap** — she/her and she/they characters: `pay_rate × 0.82` applied in `finishCreation()` after `simulateFinancialHistory()`. US median 82¢ (BLS 2023). Sector variation not modeled.
- **Job standing ceiling** — she/her characters in food_service/retail: natural decay ×1.15 (15% faster). Requires more effort to maintain the same standing. In `advanceTime()` after precarity multiplier.
- **Street safety NE** — at street/bus_stop/park after 9pm (tod > 1260 or < 360): she/her +0.4 pts/hr NE, they/them +0.2 pts/hr NE. Background hypervigilance, not a danger event.
- **Trans visibility cortisol** — in `cortisolTarget()`: +2 pts when trans + at workplace + not out. +0.8 pts when trans + location_familiarity < 0.2 (passing status not yet established). Both diminish as the character becomes comfortable/out.
- **Prose texture** — street description appends "You're aware of the distance to the next light." (she/her, late night) or "You're aware of how you're reading right now." (they/them, late night). Deterministic, no RNG.
- **Idle thoughts** — she/her at workplace: 3 thoughts ("You've explained this twice already.", "The thing is, you're right, and it doesn't matter.", "You manage down the reaction you'd have if things were equal." — last gated on high NE). Trans at workplace: 2 thoughts ("You give them the name they know.", "This version of you exists in this building.").
