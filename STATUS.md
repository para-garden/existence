...
## Interactions (183 + dynamic item interactions)
...
### Shelter (5)
check_in_shelter (!shelter_bed + displaced; 20 min; 60% bed success — stress −8 + shelter_bed=true + staying_with='shelter'; 40% turned away — stress +5, NE +6, serotonin −3; 2 RNG calls), sleep_at_shelter (shelter_bed + displaced; 8h; quality 0.85× base; stress −10; clears shelter_bed after sleep — must check in again each night; 2 RNG calls), leave_shelter (always available; 5 min; moves to street; 1 RNG call), nod_to_shelter_resident (familiar+ recognition tier + shelter_residents; 1 min; serotonin +1; 1 cosmeticRng call), talk_to_shelter_resident (regular recognition tier + shelter_residents; 8 min; serotonin +2, dopamine +1, social +2; 1 cosmeticRng call).
