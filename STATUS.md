...
### Clinic (4)
check_in_clinic (not checked in + not clinic_ready; 5 min; cortisol +3, serotonin −1; schedules clinic_ready interrupt 45–89 min out; 2 RNG calls), wait_at_clinic (checked in + not clinic_ready; 10 min; energy −2; adjustSentiment routine/irritation +0.005; 2 RNG calls), see_doctor_clinic (checked in + clinic_ready OR 45+ min elapsed; 20 min; branches on health state — dental referral + dentist scheduled + dental_ache −25; antacid prescription for gastritis; HRT script for trans+hrt_active; pain_management note for chronic_pain > 40; tapering_nicotine for quit_attempt='nicotine' + nicotine_habit > 30; tapering_alcohol for quit_attempt='alcohol' + alcohol_tolerance > 30 (high tolerance > 60: DT safety warning); general serotonin +5 cortisol −8; 3 RNG calls unconditional), leave_clinic (always available; 15 min; clears check-in state, moves to street; 1 RNG call).
State: `clinic_last_visit` (game-minutes), `clinic_checkin_time` (null or game-minutes), `clinic_ready` (bool), `clinic_prescriptions` (string[]: 'antacid' | 'hrt' | 'dental_referral' | 'pain_management' | 'tapering_nicotine' | 'tapering_alcohol').
Prescription effects: 'antacid' → gastritis_pain decays 0.1/hr faster in advanceTime(). 'hrt' → enables take_hrt interaction (already modeled). 'dental_referral' → dentist appointment scheduled on interrupt queue. 'tapering_nicotine' → NRT patches; `taperingFactor('nicotine')` returns 0.5 when supply > 0, scaling all nicotine withdrawal NT effects and craving by 0.5×. Prescribed when quit_attempt='nicotine' + nicotine_habit > 30. 'tapering_alcohol' → benzodiazepine taper; `taperingFactor('alcohol')` returns 0.5 when supply > 0, scaling all alcohol withdrawal/rebound NT effects and craving by 0.5×. High tolerance (> 60) triggers extra DT safety warning. Prescribed when quit_attempt='alcohol' + alcohol_tolerance > 30. Cannabis has no tapering pharmacotherapy — `taperingFactor('cannabis')` always returns 1.0.
