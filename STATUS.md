...
- `take_pain_medication` at apartment_bathroom — requires prescription_opioid item + doses remaining + (chronic pain > 15 OR dental pain OR opioid withdrawal). Prose arcs: first-time relief (warmth, body remembering what not-hurting feels like), established use (predictable relief), high tolerance (diminishing returns, counting doses), withdrawal-driven use (taking to stop the crawling, not for pain).
...
**Insurance:** `character.insurance_type` — `'employer'` | `'marketplace'` | `'medicaid'` | `'uninsured'`. Generated at chargen (2 charRng calls: type roll + bill day offset). Distribution derived from job_type: office (80% employer, 10% marketplace, 5% medicaid, 5% uninsured), food_service/retail (30/10/25/35), gig_worker (5/15/30/50). `healthcareCostMultiplier()` in state.js: employer 0.2, marketplace 0.4, medicaid 0.0, uninsured 1.0. Applied at all pharmacy fills ($15 sticker), HRT fills ($25 sticker), OTC pharmacy ($6/$8 sticker), ER treatment ($200 sticker), clinic illness medication ($15 sticker), dental appointment ($120 sticker). Monthly premium: employer $150, marketplace $300, medicaid/uninsured $0. Missed premium → immediate coverage lapse to uninsured. Approximation debt (insurance): US-centric model; non-US jurisdictions have public systems not captured; copay structure heavily simplified; dental insurance separate from health (not modeled); OTC typically not covered (multiplier applied anyway). `grep 'Approximation debt (insurance)'`.
...
- Rent — from backstory
- Utilities — $65 (approximation; skipped for all_inclusive housing)
...
**Phone condition:** `phone_cracked` boolean on character — cosmetic CSS crack overlay (`.phone--cracked::after`, hairline linear-gradient lines) applied to the phone overlay when true. Generated at chargen from `economic_origin`: precarious 55%, modest 30%, comfortable 8%, secure 1%. Exactly 1 charRng call. Condition affects texture, not function. `phone_age` (0–5 years, continuous) on character — derived from `economic_origin` at chargen (1 charRng call). Sets initial `phone_age_days` and `battery_health` in `applyToState()`. `phoneAgeTier()` (new/recent/aging/old/ancient) drives: (1) deterministic loading prose in `phoneScreenDescription()` at aging/old/ancient tiers, (2) battery drain scaling (`1 + age_years * 0.1`), (3) 2 phone frustration idle thoughts at old/ancient tiers when `viewing_phone`.
...
## Interactions (243 + dynamic item interactions)
...
### Shelter (7)
check_in_shelter (!shelter_bed + displaced; 20 min; bed chance modulated by time of day (evening rush −15%) and weather (rain/snow −10%); repeat visitor recognition at familiar/regular tiers; 2 RNG calls), sleep_at_shelter (shelter_bed + displaced; 8h; quality 0.85× base; stress −10; clears shelter_bed after sleep — must check in again each night; 2 RNG calls), talk_to_shelter_staff (shelter_bed + displaced + once per wake period; 5 min; serotonin +2, social +3; recognition suffix at familiar/regular; 1 RNG call), shelter_meal (shelter_bed + displaced + 6–7 PM + once per wake period; 20 min; hunger −40, social +2; recognition suffix at regular; 1 RNG call), leave_shelter (always available; 5 min; moves to street; 1 RNG call), nod_to_shelter_resident (familiar+ recognition tier + shelter_residents; 1 min; serotonin +1; 1 cosmeticRng call), talk_to_shelter_resident (regular recognition tier + shelter_residents; 8 min; serotonin +2, dopamine +1, social +2; 1 cosmeticRng call).
Named NPCs: 3 shelter residents generated at chargen (`shelter_residents[]`). Recognition-gated: familiar+ → nod interaction, regular → talk interaction. Location description includes deterministic named-resident prose at familiar/regular tiers. Idle thoughts reference residents at familiar+.
Connected to: street (10 min).

### Clinic (4)
check_in_clinic (not checked in + not clinic_ready; 5 min; cortisol +3, serotonin −1; schedules clinic_ready interrupt 45–89 min out; 2 RNG calls), wait_at_clinic (checked in + not clinic_ready; 10 min; energy −2; adjustSentiment routine/irritation +0.005; 2 RNG calls), see_doctor_clinic (checked in + clinic_ready OR 45+ min elapsed; 20 min; branches on health state — dental referral + dentist scheduled + dental_ache −25; antacid prescription for gastritis; HRT script for trans+hrt_active; pain_management note for chronic_pain > 40; illness prescription for illness_severity > 0.2; hEDS referral for heds=true; tapering_nicotine for quit_attempt='nicotine' + nicotine_habit > 30; tapering_alcohol for quit_attempt='alcohol' + alcohol_tolerance > 30 (high tolerance > 60: DT safety warning); general serotonin +5 cortisol −8; 3 RNG calls unconditional), leave_clinic (always available; 15 min; clears check-in state, moves to street; 1 RNG call).
State: `clinic_last_visit` (game-minutes), `clinic_checkin_time` (null or game-minutes), `clinic_ready` (bool), `clinic_prescriptions` (string[]: 'antacid' | 'hrt' | 'dental_referral' | 'pain_management' | 'illness' | 'heds_referral' | 'physical_therapy' | 'tapering_nicotine' | 'tapering_alcohol').
Prescription effects: 'antacid' → gastritis_pain decays 0.1/hr faster in advanceTime(). 'hrt' → enables take_hrt (gated on medication_supply['hrt'] > 0) and fill_hrt_prescription interactions; fill sets medication_supply['hrt'] = 30; supply depletes in advanceTime at 1/day. 'dental_referral' → dentist appointment scheduled on interrupt queue. 'illness' → sets illness_medicated=true when filled at pharmacy (0.4× NT effect factor). 'heds_referral' → specialist referral documented. 'physical_therapy' → enables do_pt_exercises interaction (requires pain_management already on file + chronic_pain > 30 or hEDS). 'tapering_nicotine' → NRT patches; `taperingFactor('nicotine')` returns 0.5 when supply > 0, scaling all nicotine withdrawal NT effects and craving by 0.5×. Prescribed when quit_attempt='nicotine' + nicotine_habit > 30. 'tapering_alcohol' → benzodiazepine taper; `taperingFactor('alcohol')` returns 0.5 when supply > 0, scaling all alcohol withdrawal/rebound NT effects and craving by 0.5×. High tolerance (> 60) triggers extra DT safety warning. Prescribed when quit_attempt='alcohol' + alcohol_tolerance > 30. Cannabis has no tapering pharmacotherapy — `taperingFactor('cannabis')` always returns 1.0.
...
### Phone Mode (30, triggered from phone UI)
...
- Personal calendar: `personal_calendar` on character — `CalendarEvent[]` with `{month, day, label, type}`. Family member always gets a birthday; friends get one with ~50% probability each. 9 charRng calls always (balanced across branches). Stored on character and copied to state at `applyToState()`. Calendar alerts fire as `calendar_alert` scheduled interrupts at 9:00 AM on event day. Phone calendar screen shows personal events alongside work schedule. Prose: Porpentine-toned reminders shaded by family relationship quality, serotonin, social tier.
...
- **Still deferred:** race/ethnicity depth (microaggressions), diagnostic gaps (healthcare system needed), housing discrimination, age discrimination.
