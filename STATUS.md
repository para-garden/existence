...
**Consumable / durable inventory:** `moisturizer_count` (uses), `pain_reliever_count` (tablets), `period_supply_count` (units; only relevant for characters with a uterus), `has_umbrella` (boolean durable). `needs_period_supplies` (boolean) — set when supplies run out during menstrual phase; cleared on restock or when out of flow. **Phase 1 dual-write:** flat state vars still exist; `items.js` tracks locations alongside.

- **Items** (`js/items.js`, `v1`) — inventory and container tracking. Phase 1: data model + dual-write (flat state vars remain source of truth). Typed `ITEM_DEFS` registry: phone, cigarettes, keys, umbrella, pain_reliever, moisturizer, period_supplies, alcohol, cannabis, makeup, binder. Each has `carryClass` (habitual/explicit/fixed), `defaultSpot`, `portable`, `counted`. Runtime state: `Map<type, ItemStack[]>` where each stack has `{ location, count, lastAccessed }`. Apartment spots: nightstand, bedroom_drawer, bathroom_cabinet, kitchen_counter, by_the_door, living_room_table — mapped to game rooms via `SPOT_TO_ROOM`. Special locations: `on_person`, `at_work`, `at_friends`. Per-spot `disorder` (0–1) with `accessibility()` function (recency × disorder). Queries: `itemsAtSpot()`, `itemsInRoom()`, `itemsOnPerson()`, `hasOnPerson()`, `hasAccessible()`, `countOf()`, `countAt()`, `locationOf()`, `spotDisorder()`, `accessibility()`. Mutations: `add()`, `remove()`, `removeFrom()`, `moveTo()`, `touchItem()`, `tidySpot()`, `addDisorder()`. Carry mechanics (stubs in Phase 1): `resolveLeaveHome()` (currently carries everything), `resolveSleep()` (pocket items → default spots). `reset()` populates from flat state vars after `applyToState()`. Serializable: `serialize()`/`deserialize()` with version tag. Wired in `context.js` after clothing; `reset()` called alongside dishes/linens/clothing in `game.js`.
