...
## Interactions (191 + dynamic item interactions)
...
Connected to: apartment_kitchen (2 min), bus_stop (3 min), corner_store (4 min), park (7 min), soup_kitchen (8 min), library (10 min), shelter (10 min), pharmacy (10 min), food_bank (12 min), friends_apartment (15 min), clinic (15 min), er (25 min).
...
### Pharmacy (6)
browse_pharmacy (8am–9pm; 10 min; 1 RNG call; NT-shaded browsing prose), fill_prescription (8am–9pm + has non-hrt/non-referral prescriptions; 15 min; $15/med 30-day supply; 2 RNG calls), fill_hrt_prescription (8am–9pm + hasPrescription('hrt'); 15 min; $25 30-day supply; serotonin +2; 2 RNG calls), buy_otc_pharmacy (8am–9pm; 5 min; $6 ibuprofen / $8 antacid OTC; 14-day supply; 2 RNG calls), pick_up_refill (8am–9pm + any prescription supply < 5 days; 10 min; $15/refill 30-day supply; 2 RNG calls), leave_pharmacy (always; 5 min; moves to street; 1 RNG call).
State: `pharmacy_last_fill` (game-minutes), `medication_supply` (Record<string, number> — days remaining per medication). Medication supply depletes at 1 unit/24h in advanceTime(). `illness_medicated` flag auto-clears when illness supply reaches 0.
Connected to: street (10 min).

### Emergency Room (4)
er_check_in (not checked in + not er_ready + gate: illness > 0.5 OR chronic_pain > 70 OR vasovagal_recovery > 50 OR dental abscess OR migraine > 70; 15 min; cortisol +8, NE +5; schedules er_ready interrupt 60–179 min; 2 RNG calls), wait_at_er (checked in + not er_ready; 20 min; energy −3, stress +2; 1 RNG call), er_treatment (checked in + er_ready OR 120+ min elapsed; 45 min; $200 flat — charges even if can't pay, financial_anxiety +0.08 when can't; branches: illness severity ×0.4 + medicated; vasovagal ×0.3 + NE/cortisol relief; chronic pain ×0.5; dental abscess drain + urgent dentist referral; migraine ×0.25 + NE/cortisol relief; 3 RNG calls), leave_er (always; 10 min; clears check-in state, moves to street; 1 RNG call).
State: `er_checkin_time` (null or game-minutes), `er_ready` (bool), `er_last_visit` (game-minutes).
Event: `er_ambient` — 7% chance per checkEvents when at ER. Ambient prose: machines, other patients, intercom pages, fluorescent lights. NT-shaded by NE and adenosine.
Connected to: street (25 min). Always open (no hour restriction on check-in gate — the medical gates are the constraint).
