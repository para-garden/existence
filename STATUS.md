...
State fields: `labor_arrangement` (`{type, day_pattern, work_days, shift_start, shift_end, split_shift, shift_start_2, shift_end_2, reveal_horizon_hours, reveal_tod, work_days_per_week}`) and `known_shifts` (map of absolute game-day → `{start,end,blocks?}` | null | undefined-absent).
