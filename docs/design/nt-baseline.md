...
**Substances affect baselines indirectly — and this is already handled.** The baseline drift runs every tick tracking the recent average NT level. Chronic caffeine use elevates NE → NE baseline drifts up over weeks → quitting means NE crashes below the elevated baseline → withdrawal feels worse than it would for a first-time user. This emerges automatically without special-casing. The substance withdrawal *pharmacokinetic* thresholds (`caffeine_level < 15`, `alcohol_level < 5`) remain absolute — they're anchored to real blood concentrations, not psychological experience. But their NT effects feed back into the baseline system through the ordinary drift mechanism. The rate of baseline shift is also naturally proportional to the size and frequency of substance-driven NT deviations, since larger deviations from current baseline produce faster baseline movement toward them.
