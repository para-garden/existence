...
- **Stage 1 (the content investment — highest emotional yield).** The backstory
  ...
  **Stage 1 is downstream of the simulated-life substrate** (`simulated-life.md`): the
  past these cues read from must be **genuinely generated by the world simulation** — a
  many-agent, prior-only, causes-only forward draw — **not fabricated** or back-justified
  to fit the protagonist. The episodic spine is the *output* of that substrate; until it
  exists (even coarsely), Stage 1 cues land on void or fabrication, which is exactly what
  the "the world is real / prior-not-posterior / author binds their own hands" principles
  forbid. Build order: the simulated-life substrate (at least a coarse first cut) → this
  episodic spine.
