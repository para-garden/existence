...
export function createGame(ctx) {
  ...
  async function init() {
    ...
    // v7: gym membership chargen (2 new charRng calls)
    ...
    // v28: body composition — height_cm, body_mass, waist_mass_sensitivity, hip_mass_sensitivity
    //     chargen (+3 net charRng calls, replaces abdominal_baseline); processDailyBodyMass()
    //     per-sleep drift engine in state.js
    // v29: muscle & fitness — fast_twitch_ratio, hypertrophic_response, aerobic_trainability
    //     chargen (+3 charRng calls); skeletal_muscle_mass, aerobic_capacity state vars;
    ...
    // v32: family multi-member array — replaces single family_member object with family_members[] array.
    ...
    // v34: friend + family NPC simulation — friends: replaces flavor dispatch with live personality
    //     params (warmth, openness, stability), life facts, dynamic state, trust. 8 charRng calls
    //     per friend (was 1 flavor pick). processFriendEvents() on backgroundRng each sleep cycle.
